...
Central.prototype.getCharacteristic = function(serviceId, characteristicId, callback) {
  ...
  if (this.discoveredCharacteristics[serviceId] && this.discoveredCharacteristics[serviceId][characteristicId]) {
    return callback(null, this.discoveredCharacteristics[serviceId][characteristicId]);
  }
  ...
  p.connect(function() {
    p.discoverServices([serviceId], function(serErr, services) {
      ...
      if (services.length > 0) {
        var s = services[0];
        that.discoveredCharacteristics[s.uuid] = {uuid: s.uuid};
        ...
        s.discoverCharacteristics([characteristicId], function(charErr, characteristics) {
          ...
          that.discoveredCharacteristics[s.uuid][c.uuid] = c;
          callback(null, c);
        });
      } else {
